...
with open("LISTA ESTADOS UNIDOS.M3U", "w") as f:
...
repo_urls = [
    "https://github.com/punkstarbr/STR-YT/raw/main/LISTA%20%20ITALIA.M3U"
]
...
with open("LISTA ITALIA.M3U", "w") as f:
    for channel in working_channels:
        if "extm3u_line" in channel:
            f.write(f"{channel['extm3u_line']}\n")
        else:
...
with open("LISTA JAPÃO.M3U", "w") as f:
    for channel in working_channels:
        if "extm3u_line" in channel:
            f.write(f"{channel['extm3u_line']}\n")
        else:
